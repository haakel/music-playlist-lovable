...
jQuery(document).ready(function($) {
    ...
    $('#select-cover-image').on('click', function(e) {
        ...
        coverFrame.on('select', function() {
            ...
            // Show preview
            $('#cover-preview').html('<img src="' + attachment.url + '" alt="Cover Preview">');
        });
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    const defaultCoverUrl = '../wp-content/plugins/music-playlist/assets/images/default-cover.png';
                    const coverUrl = track.cover_image ? track.cover_image : defaultCoverUrl;
                    ...
                    $button.closest('tr').fadeOut(300, function() {
                        ...
                        if ($('#music-playlist-tracks-list tr').length === 0) {
                            $('.music-playlist-tracks').html(
                                '<p>' + 'No tracks in this playlist yet. Add your first track above.' + '</p>'
                            );
                        }
                    });
                } else {
